  ...
  const upload = async () => {
    ...
    try {
      ...
      for (let index = 0; index < rows.length; index += 500) {
        const { error } = await supabase.from("reviews").insert(rows.slice(index, index + 500));
        if (error) throw error;
      }
      setStatus("success");
    } catch (error) {
      setStatus("error");
      setMessage(error instanceof Error ? error.message : "The reviews could not be uploaded.");
    }
  };
